...
export function useTreasureSearch(config) {
  ...
  const searchTreasure = useCallback(() => {
    ...
    if (scriptResult?.handled) {
      // FIX: gli effetti runtime sono in `scriptResult.effects.*`, non al
      // top-level. Senza questo accesso `forceFinishTurn` veniva ignorato e
      // un comando `fineturno` in uno script di tesoro non chiudeva mai il
      // turno, dando l'impressione che gli script non funzionassero.
