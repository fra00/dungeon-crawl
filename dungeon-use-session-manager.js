...
export function useDungeonSessionManager({
  ...
  const moveCurrentHeroTo = useCallback((nextX, nextY, baseSession) => {
    return commitSessionUpdate((providedSession) => {
      ...
      const activeHero = sourceSession.heroes?.find(h => h.turnOrder === sourceSession.currentTurn);
  ...
  const resolveTrapEffectOnCurrentHero = useCallback((trapType, rockFallX, rockFallY) => {
    commitSessionUpdate((providedSession) =>
      applyTrapEffectOnCurrentHeroSession(providedSession, trapType, rockFallX, rockFallY)
    );
    return true;
  }, [commitSessionUpdate]);
