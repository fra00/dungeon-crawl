...
export function useDungeonMapQuery({ gameSession, visibilityMap }) {
    ...
    // NOTA: `mobili.flpo` e `mobili.flpv` sono flag puramente visivi (mirror
    // dell'immagine del mobile) e NON devono influire sul blocco del movimento.
    // Una cella è bloccata da arredo se contiene un mobile (`mobili.num != null`)
    // oppure se ha la flag esplicita `arnt.inv` (invalicabile per altri motivi —
    // es. trono "sigillato" o decorazione speciale). Vedi anche il commento in
    // [.cursor/rules/coordinates.mdc](mdc:.cursor/rules/coordinates.mdc).
    //
    // Bug storico: la versione precedente includeva `flpo === true || flpv === true`
    // nella condizione, e questo bloccava in modo spurio celle perfettamente
    // calpestabili in mappe legacy come `DGBase01.json` (vedi cella `(13,12)` con
    // `mobili.num=null` ma `mobili.flpv=true`). Sintomo principale segnalato:
    // pathfinding incapace di terminare su cella adiacente a una porta.
    const isBlockedByFurniture = useCallback((x, y) => {
        const cell = getMapCell(x, y);
        if (!cell) return false;
        return cell?.mobili?.num != null || cell?.arnt?.inv === true;
    }, [getMapCell]);
